...
void CGenerator::translate__statements(CNode *node, std::ofstream &output,
  ...
  if (statement->name == "return") {
    ...
    auto s = current_index_table_->getIndex(
    ...
    int d = 0;
    translate_CNode(statement->children[2], output, d);
    output << "   iinc " << s.first << " 1\n";
    output << "   goto FOR" << s.first << "\n";
    output << "   EXIT" << s.first << ":\n";
    current_ct_ = current_ct_->getParent();
    ...
    if (statement->children[2] != nullptr) {
      scope++;
      current_ct_ =
          current_ct_->getParent()->getSubScopeTable(std::to_string(scope));
      int b = 0;
